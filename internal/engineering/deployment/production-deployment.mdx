...
<Steps>
  ...
  <Step title="Check Production Status">
    ...
    qumis lambda list-functions --env prod | grep api
    ```
  </Step>
...
<Tabs>
  <Tab title="API Service">
    ...
    The `--confirm "prod"` flag is required as a safety measure.
  </Tab>
  ...
  <Tab title="LLM Service">
    ...
    qumis deploy llm \
      --deploy-branch "main" \
      --env "prod" \
      --reason "Update LLM models - Ticket ENG-125" \
      --workflow-ref "main" \
      --confirm "prod"
    ```
  </Tab>
...
<Steps>
  <Step title="Watch GitHub Actions">
    ...
    # Navigate to Actions tab to watch the deployment
    ```
  </Step>
...
|------|-------------|---------|
| `--deploy-branch` | Git branch to deploy | `"main"` or `"hotfix-123"` |
| `--env` | Target environment | `"prod"` |
| `--reason` | Audit log reason | `"Deploy feature XYZ"` |
| `--workflow-ref` | GitHub workflow branch | `"main"` |
| `--confirm` | Confirmation (prod only) | `"prod"` |
...
| qa | 340452546718 | No |
| uat | 499854674638 | No |
| **prod** | **729033428609** | **Yes** (`--confirm prod`) |
