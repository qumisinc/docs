...
<Frame>
  <video
    ...
    controls
    src="/videos/efficiently-resolving-git-branch-conflicts.mp4"
    title="Efficiently resolving git branch conflicts"
  />
</Frame>
...
<Info>
**Rebase** creates a clean linear history. **Merge** preserves the exact timeline without requiring force push.
</Info>
...
<Steps>
  <Step title="Update your local release branch">
    ```bash
    git checkout release
    git pull origin release
    ```
  ...
  <Step title="Switch to your feature branch">
    ```bash
    git checkout your-feature-branch  # e.g., erik/eng-984-add-auth
    ```
  </Step>

  <Step title="Rebase onto the updated release">
    ```bash
    git rebase release
    ```
  </Step>

  <Step title="Force push safely">
    ```bash
    git push --force-with-lease
    ```
    <Warning>
...
<Steps>
  <Step title="Fix the conflicted files">
    Your editor will show `<<<<` markers indicating conflicts. Review and resolve each conflict.
  </Step>

  <Step title="Stage the fixed files">
    ```bash
    git add .
    ```
  </Step>

  <Step title="Continue rebasing">
    ```bash
    git rebase --continue
    ```
  </Step>

  <Step title="Repeat if more conflicts appear">
...
<Steps>
  ...
  <Step title="Merge release into your branch">
    ```bash
    git merge release
    ```
  </Step>
