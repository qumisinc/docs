...
qumis deploy api --env prod --reason "Production release" --confirm prod
...
qumis services exec api --env prod --reason "Debug" --confirm prod -- rails c
...
<Tabs>
  ...
  <Tab title="Production">
    ```bash
    # Production requires confirmation and reason
...
qumis services exec api --env prod --reason "Apply migration" --confirm prod -- rails db:migrate
...
| `--confirm` | Confirmation for production | `prod` |
...
| `prod` | 729033428609 | qumis_prod | Yes |
...
```bash
# Open in browser
qumis gh view         # Current repo and branch
...
qumis repo pack --output context.xml

# Copy to clipboard (macOS)
qumis repo pack --stdout | pbcopy
```
