---
title: "Welcome to Qumis"
description: "Expert AI built for the top minds in insurance. Attorney-trained AI that delivers clarity, speed, and legal-grade reasoning for the industry's most complex risks."
---

# Expert AI for Insurance Professionals

**Qumis** is attorney-trained AI that delivers clarity, speed, and legal-grade reasoning for the insurance industry's most complex risks. We eliminate manual work, minimize errors, and deliver attorney-level insights for brokers, claims leaders, attorneys, and risk professionals.

<Info>
  **SOC 2 Type I Certified** - Enterprise-grade security with complete data privacy. Your client data is never used for AI training.
</Info>

## Why Insurance Professionals Choose Qumis

- **Accurate**: Attorney-level output with legal-grade reasoning capabilities
- **Bespoke**: AI learns with use, becoming more personalized to your workflows
- **Private & Secure**: Complete data privacy with enterprise-grade encryption
- **Fast**: Eliminate manual work and deliver insights in seconds, not hours

---

## Get Started

Learn how to successfully implement Qumis in your organization—from building your strategic roadmap and managing change to configuring secure access for your team.

<Columns cols={2}>
  <Card
    title="Step 1: Plan Your Strategy"
    icon="lightbulb"
    href="/get-started/foundations-for-success/implementing-ai"
  >
    **Strategic planning & change management** • 30-45 min read
    
    Build your implementation roadmap with proven frameworks for AI adoption, use case identification, and change management.
  </Card>
  <Card
    title="Step 2: Configure Your Environment"
    icon="key"
    href="/get-started/setup-and-access/single-sign-on"
  >
    **Technical configuration & onboarding** • 15-20 min read
    
    Get your team up and running with enterprise-grade security and seamless access management.
  </Card>
</Columns>

---

## Features

Access powerful AI tools that supercharge your daily insurance work—chat with documents, organize files securely, save proven prompts, and generate professional reports.

<Columns cols={3}>
  <Card
    title="Chats"
    icon="message-square-text"
    href="/features/chats"
  >
    Interactive analysis of insurance documents with natural language queries.
  </Card>
  <Card
    title="Document Vault"
    icon="vault"
    href="/features/document-vault"
  >
    Private, encrypted storage with instant document reusability across workflows.
  </Card>
  <Card
    title="Legal Search"
    icon="search"
    href="/features/legal-search"
  >
    Advanced search for insurance-specific legal content and case law.
  </Card>
  <Card
    title="Prompt Library"
    icon="library"
    href="/features/prompt-library"
  >
    Pre-built prompts and templates to accelerate your insurance analysis workflows.
  </Card>
  <Card
    title="Reports"
    icon="file-text"
    href="/features/reports"
  >
    Generate comprehensive analysis reports with attorney-level insights.
  </Card>
</Columns>

---

## Reports

Generate comprehensive, attorney-level analysis in minutes—compare policies side-by-side, assess claim coverage, verify contract compliance, and create custom reports tailored to your exact needs.

<Columns cols={2}>
  <Card
    title="Claims"
    icon="gavel"
    href="/reports/claims"
  >
    Legal-grade reasoning for coverage determination and claims resolution.
  </Card>
  <Card
    title="Comparisons"
    icon="files"
    href="/reports/comparisons"
  >
    Side-by-side comparison of quotes, policies, and contracts with strategic insights.
  </Card>
  <Card
    title="Contract Review"
    icon="file-signature"
    href="/reports/contract-review"
  >
    Comprehensive contract analysis to identify obligations and requirements.
  </Card>
  <Card
    title="Coverage Table"
    icon="table"
    href="/reports/coverage-table"
  >
    Organized coverage summaries for quick reference and comparison.
  </Card>
  <Card
    title="Custom Reports"
    icon="chart-bar"
    href="/reports/custom-reports"
  >
    Build tailored reports for your specific insurance analysis needs.
  </Card>
  <Card
    title="Policy Analysis"
    icon="file-search"
    href="/reports/policy-analysis"
  >
    Comprehensive policy analysis to identify key coverage details and exclusions.
  </Card>
</Columns>

---

## Playbooks

Learn practical strategies and proven workflows designed for brokers, producers, claims professionals, account managers, and service teams—showing exactly how to win placements, resolve claims faster, and deliver exceptional service.

<Columns cols={3}>
  <Card
    title="Account Managers"
    icon="user-tie"
    href="/playbooks/for-account-managers"
  >
    Streamline client relationships and policy management workflows.
  </Card>
  <Card
    title="Brokers"
    icon="handshake"
    href="/playbooks/for-brokers"
  >
    Win more placements with strategic quote evaluation and competitive insights.
  </Card>
  <Card
    title="Claims Professionals"
    icon="clipboard-check"
    href="/playbooks/for-claims-professionals"
  >
    Accelerate claim resolutions with attorney-level policy interpretation.
  </Card>
  <Card
    title="Producers"
    icon="users"
    href="/playbooks/for-producers"
  >
    Close deals faster with comprehensive policy analysis and client advisory.
  </Card>
  <Card
    title="Service Teams"
    icon="headset"
    href="/playbooks/for-service-teams"
  >
    Deliver exceptional client service with rapid policy insights and coverage verification.
  </Card>
</Columns>

---

## Resources

Get help, stay updated, and troubleshoot any issues you encounter.

<Columns cols={3}>
  <Card
    title="Contact Support"
    icon="life-ring"
    href="/resources/contact-support"
  >
    Reach our support team for assistance with any questions or issues.
  </Card>
  <Card
    title="Product Updates & Release Notes"
    icon="rocket"
    href="/resources/product-updates-and-release-notes"
  >
    Stay informed about the latest features, improvements, and bug fixes.
  </Card>
  <Card
    title="Troubleshooting & FAQs"
    icon="circle-question"
    href="/resources/troubleshooting-and-faqs"
  >
    Find answers to common questions and resolve issues quickly.
  </Card>
</Columns>

---

<Card
  title="Request a Demo"
  icon="calendar"
  href="https://calendly.com/qumis-dan/demo?background_color=f1f0ed&text_color=242423&primary_color=4c94fb"
>
  See Qumis in action with a personalized demo for your insurance use cases.
</Card>